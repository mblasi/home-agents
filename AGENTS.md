...
Un hook de Stop en `.opencode/settings.json` / `.claude/settings.local.json` recuerda esto automáticamente.
...
### Desplegar desde el agente (mismo camino que el cloud-bo)
...
mismo funnel que el bridge usa al desencolar un comando de la nube. **Desplegar desde el agente
(OpenCode/Claude) es idéntico a hacerlo desde el cloud-bo**; sólo cambia el disparador (LAN/SSH vs cola en Firestore).
